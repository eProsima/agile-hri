...
class Tuning:
    ...
    def save_config(self, filename):
        ...
        for param in PARAMETERS_WRITE:
            config[param] = self.read(param)

        with open(filename, 'w') as f:
            json.dump(config, f, indent=4)
        print(f"Save mic config file in: {filename}")
    ...
    def load_config(self, filename):
        ...
        for param, value in config.items():
            if param == 'AGCONOFF':
                if value == 1:
                    print(f"Param '{param}' is {value}, so Automatic Gain Control is ON. AGCGAIN will be automatically changed during runtime.")
                else:
                    print(f"Param '{param}' is {value}, so Automatic Gain Control is OFF. AGCGAIN will remain constant during runtime.")
            if self.read(param) != value:
                self.write(param, value)
                print(f"Param '{param}' changed to: {value}")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '-p':
            ...
            if sys.argv[1] == '-r':
                ...
                if len(sys.argv) == 3:
                    dev.save_config(sys.argv[2])
                else:
                    print('Invalid number of arguments. Usage: python tuning.py -s <filename.json>')
            elif sys.argv[1] == '-l':
                if len(sys.argv) == 3:
                    dev.load_config(sys.argv[2])
                else:
                    print('Invalid number of arguments. Usage: python tuning.py -l <filename.json>')
            else:
