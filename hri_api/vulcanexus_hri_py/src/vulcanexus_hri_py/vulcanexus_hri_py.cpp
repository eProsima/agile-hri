...
#include "vulcanexus_hri_py/converters.hpp"
#include "vulcanexus_hri_py/ndarray_converter.h"
...
public:
    ...
    create(std::string node_name, bool auto_spin, bool use_sim_time)
    {
        ...
        auto node_sp = rclcpp::Node::make_shared(node_name, options);
        hri::NodeLikeInterfaces node_like(*node_sp);
        return std::shared_ptr<PyHRIListener>(
            new PyHRIListener(node_like, auto_spin));
    }
...
protected:
    explicit PyHRIListener(hri::NodeLikeInterfaces node_like, bool auto_spin)
        : hri::HRIListener(node_like), node_base_interface_(node_like.get_node_base_interface())
    {
        executor_ = rclcpp::executors::MultiThreadedExecutor::make_unique();
...
PYBIND11_MODULE(hri, m)
{
  m.doc() =
    R"(
    Python wrapper library to the Vulcanexus HRI framework, based on ROS4HRI (https://www.ros.org/reps/rep-0155.html) bindings.
  ...
  body.doc() =
    ...
    - :py:attr:`skeleton3d` -- 3D skeleton keypoints (Dict[SkeletalKeypoint, PointOfInterest3D])
    )";
  ...
  body.def_property_readonly(
    "skeleton3d", &hri::Body::skeleton3d,
    "3D skeleton keypoints (Dict[SkeletalKeypoint, PointOfInterest3D])");
  ...
  hri_listener.doc() =
    ...
    It will spawn a ROS node and use it to subscribe to all the Vulcanexus HRI topics.
