...
class SkeletonNode : public rclcpp::Node
{
public:

    SkeletonNode(std::string topic_name = "/test_image")
        : rclcpp::Node("hri_cpp_example_skeleton"), topic_name_(topic_name) {}
    ...
    void init()
    {
        ...
        image_sub_ = create_subscription<sensor_msgs::msg::Image>(
            ...
            std::bind(&SkeletonNode::onImage, this, std::placeholders::_1));

        timer_ = create_wall_timer(10ms, std::bind(&SkeletonNode::timer_callback, this));
    }
    ...
    void timer_callback()
    {
        ...
        for (auto const& [body_id, body] : bodies)
        {
            ...
            if (auto landmarks = body->skeleton())
            {
        ...
        char* argv[])
{
    ...
    auto node = std::make_shared<SkeletonNode>();
    node->init();
